...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (menuContainer) {
        db.collection("rv_restaurantMenu").orderBy("category").onSnapshot(snapshot => {
            ...
            if (items.length > 0) {
                ...
                let gridHtml = '<div class="menu-grid-container">';
                
                // Add an "All" tab
                tabsHtml += `<button class="menu-tab active" onclick="filterMenu('All')">All</button>`;
                
                for (const category in categorized) {
                    tabsHtml += `<button class="menu-tab" onclick="filterMenu('${category}')">${category}</button>`;
                    
                    categorized[category].forEach(item => {
                        // Simple veg/non-veg inference based on name
                        const isNonVeg = /chicken|mutton|beef|fish|pork|egg|prawn/i.test(item.name + ' ' + (item.description || ''));
                        const badgeClass = isNonVeg ? 'menu-item-nonveg-badge' : 'menu-item-veg-badge';
                        const badgeLabel = isNonVeg ? 'Non-Veg' : 'Vegetarian';
                        const tagClass = isNonVeg ? 'menu-item-tag non-veg' : 'menu-item-tag';
                        ...
                        gridHtml += `
                            <div class="menu-item" data-category="${category}">
                                ...
                                <div class="menu-item-header">
                                    ...
                                    <div class="${badgeClass}" title="${badgeLabel}"></div>
                                </div>
                                <p class="menu-item-desc">${item.description || ''}</p>
                                <div class="menu-item-footer">
                                    <span class="menu-item-price">${item.price}</span>
                                    <span class="${tagClass}">${badgeLabel}</span>
                                </div>
                            </div>
                        `;
                    });
    ...
    db.collection("rv_siteContent").doc("trustBanner").onSnapshot(doc => {
        if(doc.exists) {
            ...
            const gs = document.getElementById('trust-google-sub'); if(gs && d.googleSub) gs.textContent = d.googleSub;
            const tt = document.getElementById('trust-trip-text'); if(tt && d.tripText) tt.textContent = d.tripText;
            const ts = document.getElementById('trust-trip-sub'); if(ts && d.tripSub) ts.textContent = d.tripSub;
            const t3t = document.getElementById('trust-third-text'); if(t3t && d.thirdText) t3t.textContent = d.thirdText;
            const t3s = document.getElementById('trust-third-sub'); if(t3s && d.thirdSub) t3s.textContent = d.thirdSub;
        }
    });
